...
class ShipmentService {
    constructor() {
        this.RATE_PER_KM_STANDARD = 100; // FCFA
        this.RATE_PER_KM_EXPRESS = 200;
        this.RATE_PER_KM_SAME_DAY = 350;
        this.BASE_RATE = 500;
        this.INSURANCE_RATE = 0.02; // 2% of declared value
    }
    /**
     ...
     */
    async estimateShipment(data) {
        const distance = this.calculateDistance(data.senderLatitude, data.senderLongitude, data.receiverLatitude, data.receiverLongitude);
    ...
    async getShipmentById(shipmentId) {
        ...
        const result = await database_1.default.query(query, [shipmentId]);
    ...
    async trackByNumber(trackingNumber) {
        ...
        const result = await database_1.default.query(query, [trackingNumber]);
        return result.rows[0] || null;
    }
    /**
