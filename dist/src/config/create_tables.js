...
let tableQueries = [
    ...
    //Profile access rights 
    {
        query: `CREATE TABLE IF NOT EXISTS "${tbl.kProfileAccessRights}" (
                profile_id INT NOT NULL REFERENCES "${table_names_1.kProfile}"(id) ON DELETE CASCADE,
...
async function createTables() {
    try {
        for (const tableQuery of tableQueries) {
            await pgdb_1.default.none(tableQuery.query, tableQuery.arguments);
        }
        access_rights_repository_1.accessRightsRepository.setUpAccessRights(); // Set up access rights after creating tables
