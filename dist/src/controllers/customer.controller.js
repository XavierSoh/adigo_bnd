...
class CustomerController {
        ...
        try {
            ...
            if (!customer.first_name) {
                return res.status(400).json({
                    status: false,
                    message: i18n_1.I18n.t('first_name_required', lang),
                    code: 400
                });
            }
                ...
                return res.status(400).json({
                    status: false,
                    message: i18n_1.I18n.t('last_name_required', lang),
                    code: 400
                });
            }
                ...
                return res.status(400).json({
                    status: false,
                    message: i18n_1.I18n.t('email_or_phone_required', lang),
                    code: 400
                });
            }
            ...
            if (!emailRegex.test(customer.email)) {
                res.status(400).json({
                    status: false,
                    message: i18n_1.I18n.t('invalid_email', lang),
                    code: 400
                });
                return;
            }
            const result = await customer_repository_1.CustomerRepository.create(customer);
            if (result.status && result.message) {
                result.message = i18n_1.I18n.t('customer_created', lang);
            }
    ...
    static async getById(req, res) {
        try {
            ...
            const result = await customer_repository_1.CustomerRepository.findById(id);
            console.log(`📥 [CustomerController] Result from repository:`, JSON.stringify(result));
            if (!result.status) {
                result.message = i18n_1.I18n.t('customer_not_found', lang);
            }
    ...
    static async getAll(req, res) {
        try {
            ...
            const result = await customer_repository_1.CustomerRepository.findAll(include_deleted === 'true');
            res.status(result.code).json(result);
        }
        catch (error) {
            res.status(500).json({
    ...
    static async update(req, res) {
        try {
            ...
            const result = await customer_repository_1.CustomerRepository.update(id, customer);
    ...
    static async softDelete(req, res) {
        try {
            ...
            const result = await customer_repository_1.CustomerRepository.softDelete(id, deleted_by);
            res.status(result.code).json(result);
        }
        catch (error) {
            res.status(500).json({
    ...
    static async restore(req, res) {
        try {
            ...
            const result = await customer_repository_1.CustomerRepository.restore(id);
            res.status(result.code).json(result);
        }
        catch (error) {
            res.status(500).json({
    ...
    static async delete(req, res) {
        try {
            ...
            const result = await customer_repository_1.CustomerRepository.delete(id);
            res.status(result.code).json(result);
        }
        catch (error) {
            res.status(500).json({
    ...
    static async search(req, res) {
        try {
            const { q, account_status, customer_tier, email_verified, phone_verified, min_loyalty_points, city } = req.query;
            const filters = {};
            ...
            if (min_loyalty_points) {
                filters.minLoyaltyPoints = parseInt(min_loyalty_points);
            }
            if (city && typeof city === 'string') {
                filters.city = city;
            }
            const result = await customer_repository_1.CustomerRepository.search(filters);
            res.status(result.code).json(result);
        }
        catch (error) {
            res.status(500).json({
    ...
    static async login(req, res) {
        try {
            ...
            const result = await customer_repository_1.CustomerRepository.authenticate(email_or_phone, password);
            // Traduire le message de réponse
            if (result.status) {
                result.message = i18n_1.I18n.t('customer_login_success', lang);
            }
            else {
                result.message = i18n_1.I18n.t('customer_login_failed', lang);
            }
            res.status(result.code).json(result);
        }
        catch (error) {
    ...
    static async updateLoyaltyPoints(req, res) {
        try {
            ...
            const result = await customer_repository_1.CustomerRepository.updateLoyaltyPoints(id, points);
            res.status(result.code).json(result);
        }
        catch (error) {
            res.status(500).json({
    ...
    static async verifyEmail(req, res) {
        try {
            ...
            const result = await customer_repository_1.CustomerRepository.verifyEmail(id);
            res.status(result.code).json(result);
        }
        catch (error) {
            res.status(500).json({
    ...
    static async verifyPhone(req, res) {
        try {
            ...
            const result = await customer_repository_1.CustomerRepository.verifyPhone(id);
            res.status(result.code).json(result);
        }
        catch (error) {
            res.status(500).json({
    ...
    static async getStatistics(req, res) {
        try {
            const result = await customer_repository_1.CustomerRepository.getStatistics();
            res.status(result.code).json(result);
        }
        catch (error) {
            res.status(500).json({
    ...
    static async bulkCreate(req, res) {
        try {
            ...
            const result = await customer_repository_1.CustomerRepository.bulkCreate(customers);
            res.status(result.code).json(result);
        }
        catch (error) {
            res.status(500).json({
    ...
    static async bulkCreateSample(req, res) {
        try {
            ...
            const result = await customer_repository_1.CustomerRepository.bulkCreate(sampleCustomers);
            res.status(result.code).json(result);
        }
        catch (error) {
            res.status(500).json({
    ...
    static async updateProfilePicture(req, res) {
        try {
            ...
            if (!req.file) {
                res.status(400).json({
                    status: false,
                    message: i18n_1.I18n.t('no_file_uploaded', lang) || 'No file uploaded',
                    code: 400
                });
                return;
            }
            ...
            const result = await customer_repository_1.CustomerRepository.update(id, {
    ...
    static async getCustomerBookings(req, res) {
        try {
            ...
            const result = await booking_repository_1.BookingRepository.findByUser(id);
