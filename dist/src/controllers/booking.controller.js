...
class BookingController {
    ...
    static async createMultiple(req, res) {
        try {
            ...
            if (!generated_trip_id || !customer_id || !Array.isArray(seats) || seats.length === 0) {
                res.status(400).json({
                    status: false,
                    message: i18n_1.I18n.t('required_fields', req.lang),
                    code: 400
                });
                return;
            }
            ...
            const customerData = await customer_repository_1.CustomerRepository.findById(customer_id);
            if (!customerData.status) {
                res.status(customerData.code).json(customerData);
                return;
            }
            const customerTier = customerData.body.customer_tier || 'regular';
            // Fetch trip price from database (source of truth)
            const tripPriceQuery = await pgdb_1.default.oneOrNone(`SELECT t.price
                 FROM trip t
                 JOIN generated_trip gt ON gt.trip_id = t.id
                 WHERE gt.id = $1`, [generated_trip_id]);
            ...
            if (payment_method === 'wallet') {
                ...
                for (const seat of seats) {
                    const available = await booking_repository_1.BookingRepository.checkSeatAvailability(generated_trip_id, seat.generated_trip_seat_id);
                    if (!available.body || !available.body.available) {
                    ...
                    const discount = (0, tier_config_1.calculateTierDiscount)(baseTripPrice, customerTier);
                    const discountedPrice = baseTripPrice - discount;
                    estimatedTotal += discountedPrice;
                }
                // Get current wallet balance
                const balanceCheck = await customer_repository_1.CustomerRepository.getWalletBalance(customer_id);
                ...
                if (currentBalance < estimatedTotal) {
                    res.status(400).json({
                        status: false,
                        message: i18n_1.I18n.t('insufficient_wallet_balance', req.lang, {
            ...
            for (const seat of seats) {
                ...
                const result = await booking_repository_1.BookingRepository.create(booking);
                if (!result.status) {
                    res.status(500).json({ status: false, message: "Erreur lors de la réservation", code: 500 });
                    return;
                }
                const bookingResult = result.body;
                ...
                if (seat.name || seat.phone || seat.document_type || seat.document_number) {
                    await booking_repository_1.BookingRepository.addPassenger(bookingResult.id, seat.name, seat.phone, seat.document_type, seat.document_number);
                }
            ...
            if (payment_method === 'wallet' && totalPrice > 0) {
                const paymentResult = await wallet_repository_1.WalletRepository.recordPayment(customer_id, totalPrice, `Booking payment for ${bookings.length} seat(s) - Group ${group_id}`);
                if (!paymentResult.status) {
                    // If payment fails, we should ideally rollback bookings
                    // For now, just return error
                    res.status(500).json({
                        status: false,
                        message: paymentResult.message || i18n_1.I18n.t('bookings_created_wallet_error', req.lang),
                        code: 500
                    });
                    return;
                }
            ...
            const detailedBookings = await booking_repository_1.BookingRepository.findByGroupId(group_id);
            ...
            socket_service_1.SocketService.broadcastNewBooking(bookingData).catch(err => console.error('Error broadcasting booking:', err));
            res.status(201).json({
                status: true,
                message: i18n_1.I18n.t('bookings_created', req.lang),
    ...
    static async create(req, res) {
        try {
            ...
            const availabilityCheck = await booking_repository_1.BookingRepository.checkSeatAvailability(booking.generated_trip_id, booking.generated_trip_seat_id);
            if (availabilityCheck.body && !availabilityCheck.body.available) {
                res.status(409).json({
                    status: false,
            ...
            const customerData = await customer_repository_1.CustomerRepository.findById(booking.customer_id);
            if (!customerData.status) {
                res.status(customerData.code).json(customerData);
                return;
            }
            ...
            if (booking.total_price && booking.total_price > 0) {
                const discount = (0, tier_config_1.calculateTierDiscount)(booking.total_price, customerTier);
            ...
            if (booking.payment_method === 'wallet' && booking.total_price > 0) {
                const createdBooking = result.body;
                const paymentResult = await wallet_repository_1.WalletRepository.recordPayment(booking.customer_id, booking.total_price, `Booking payment - ${createdBooking.booking_reference || 'Ref: ' + createdBooking.id}`);
                if (!paymentResult.status) {
    ...
    static async getAll(req, res) {
        try {
            const { agency_id, trip_id, user_id, status, group_id, with_details, include_deleted } = req.query;
            let result;
            console.log(`Agency ID |||||||____ ${agency_id}`);
            if (with_details === 'true') {
                result = await booking_repository_1.BookingRepository.findAllWithDetails(agency_id ? parseInt(agency_id) : undefined);
            }
            else if (group_id) {
                result = await booking_repository_1.BookingRepository.findByGroupId(group_id);
            }
            else if (agency_id) {
                result = await booking_repository_1.BookingRepository.findByAgency(parseInt(agency_id));
            }
            else if (trip_id) {
                if (status) {
                    result = await booking_repository_1.BookingRepository.findByTripAndStatus(parseInt(trip_id), status);
                }
                else {
                    result = await booking_repository_1.BookingRepository.findByTrip(parseInt(trip_id));
                }
            }
            else if (user_id) {
                result = await booking_repository_1.BookingRepository.findByUser(parseInt(user_id));
            }
            else if (status) {
                result = await booking_repository_1.BookingRepository.findByStatus(status);
            }
            else {
                result = await booking_repository_1.BookingRepository.findAll(include_deleted === 'true');
            }
            res.status(result.code).json(result);
        }
        catch (error) {
    ...
    static async update(req, res) {
        try {
            ...
            if (booking.generated_trip_seat_id && booking.generated_trip_id) {
                const availabilityCheck = await booking_repository_1.BookingRepository.checkSeatAvailability(booking.generated_trip_id, booking.generated_trip_seat_id, id);
                if (availabilityCheck.body && !availabilityCheck.body.available) {
                    res.status(409).json({
                        status: false,
                        message: "Ce siège est déjà réservé pour ce voyage",
            ...
            const result = await booking_repository_1.BookingRepository.update(id, booking);
            res.status(result.code).json(result);
        }
        catch (error) {
            res.status(500).json({
    ...
    static async softDelete(req, res) {
        try {
            ...
            const result = await booking_repository_1.BookingRepository.softDelete(id, deleted_by);
            res.status(result.code).json(result);
        }
        catch (error) {
            res.status(500).json({
    ...
    static async restore(req, res) {
        try {
            ...
            const result = await booking_repository_1.BookingRepository.restore(id);
            res.status(result.code).json(result);
        }
        catch (error) {
            res.status(500).json({
    ...
    static async getBookedSeats(req, res) {
        try {
            ...
            const result = await booking_repository_1.BookingRepository.getBookedSeatIds(parseInt(trip_id));
            res.status(result.code).json(result);
        }
        catch (error) {
            res.status(500).json({
    ...
    static async checkSeatAvailability(req, res) {
        try {
            ...
            const result = await booking_repository_1.BookingRepository.checkSeatAvailability(parseInt(trip_id), parseInt(seat_id), exclude_booking_id ? parseInt(exclude_booking_id) : undefined);
            res.status(result.code).json(result);
        }
        catch (error) {
            res.status(500).json({
    ...
    static async getStatistics(req, res) {
        try {
            ...
            const result = await booking_repository_1.BookingRepository.getStatistics(agency_id ? parseInt(agency_id) : undefined);
            res.status(result.code).json(result);
        }
        catch (error) {
            res.status(500).json({
    ...
    static async getRevenueStatistics(req, res) {
        try {
            ...
            const result = await booking_repository_1.BookingRepository.getRevenueStatistics(agency_id ? parseInt(agency_id) : undefined, start_date ? new Date(start_date) : undefined, end_date ? new Date(end_date) : undefined);
            res.status(result.code).json(result);
        }
        catch (error) {
            res.status(500).json({
    ...
    static async search(req, res) {
        try {
            const { customer_name, departure_city, arrival_city, status, start_date, end_date, agency_id, customer, payment_reference } = req.query;
            const filters = {};
            if (customer_name || customer) {
                filters.customerName = (customer_name || customer);
            }
            if (departure_city)
                filters.departureCity = departure_city;
            if (arrival_city)
                filters.arrivalCity = arrival_city;
            if (status)
            ...
            if (agency_id)
                filters.agencyId = parseInt(agency_id);
            const result = await booking_repository_1.BookingRepository.search(filters);
            res.status(result.code).json(result);
        }
        catch (error) {
            res.status(500).json({
    ...
    static async getByDateRange(req, res) {
        try {
            ...
            if (date) {
                ...
                const result = await booking_repository_1.BookingRepository.findByDateRange(startOfDay, endOfDay);
                res.status(result.code).json(result);
                return;
            }
            const result = await booking_repository_1.BookingRepository.findByDateRange(new Date(start_date), new Date(end_date));
            res.status(result.code).json(result);
        }
        catch (error) {
            res.status(500).json({
    ...
    static async getRecent(req, res) {
        try {
            ...
            const result = await booking_repository_1.BookingRepository.findRecent(limit ? parseInt(limit) : 10, agency_id ? parseInt(agency_id) : undefined);
            res.status(result.code).json(result);
        }
        catch (error) {
            res.status(500).json({
    ...
    static async cancelBatch(req, res) {
        try {
            ...
            const result = await booking_repository_1.BookingRepository.cancelBatch(booking_ids, cancellation_reason);
            res.status(result.code).json(result);
        }
        catch (error) {
            res.status(500).json({
    ...
    static async cleanup(req, res) {
        try {
            ...
            const result = await booking_repository_1.BookingRepository.cleanupSoftDeleted(older_than_days ? parseInt(older_than_days) : 30);
            res.status(result.code).json(result);
        }
        catch (error) {
            res.status(500).json({
    ...
    static async getSoftDeleted(req, res) {
        try {
            const { agency_id } = req.query;
            const result = await booking_repository_1.BookingRepository.findAll(true);
            // Filter only soft deleted
            if (result.status && result.body && Array.isArray(result.body)) {
                result.body = result.body.filter((booking) => booking.is_deleted === true);
            }
            res.status(result.code).json(result);
        }
        catch (error) {
     ...
     */
    static async cancelSingle(req, res) {
        try {
            ...
            // Get booking details
            const bookingResult = await booking_repository_1.BookingRepository.findById(parseInt(booking_id));
            if (!bookingResult.status || !bookingResult.body) {
                res.status(404).json({
                    status: false,
                    message: i18n_1.I18n.t('booking_not_found', req.lang),
                    code: 404
                });
                return;
            }
            const booking = bookingResult.body;
            // Verify booking belongs to customer
            if (booking.customer_id !== customer_id) {
                res.status(403).json({
                    status: false,
                    message: i18n_1.I18n.t('unauthorized', req.lang),
                    code: 403
                });
                return;
            }
            ...
            if (status !== 'confirmed' && status !== 'pending') {
                res.status(400).json({
                    status: false,
                    message: i18n_1.I18n.t('cannot_cancel_booking', req.lang) || 'Cette réservation ne peut pas être annulée',
                    code: 400
                });
                return;
            }
            ...
            if (booking.group_id) {
                ...
                const groupBookingsResult = await pgdb_1.default.manyOrNone(`SELECT id, status FROM booking WHERE group_id = $1 AND is_deleted = false`, [booking.group_id]);
            ...
            const result = await booking_repository_1.BookingRepository.cancelBatch(bookingIds, cancellation_reason);
            res.status(result.code).json(result);
        }
        catch (error) {
     ...
     */
    static async modifySingle(req, res) {
        try {
            const { booking_id } = req.params;
            ...
            if (booking.status.toLowerCase() !== 'confirmed') {
                res.status(400).json({
                    status: false,
                    message: i18n_1.I18n.t('cannot_modify_booking', req.lang) || 'Cette réservation ne peut pas être modifiée',
                    code: 400
                });
                return;
            }
            // Check if departure time is at least 2 hours in the future
            const departureTimeResult = await pgdb_1.default.oneOrNone(`SELECT actual_departure_time
                 FROM generated_trip
                 WHERE id = $1`, [booking.generated_trip_id]);
            if (departureTimeResult) {
                ...
                if (hoursUntilDeparture < 2) {
                    res.status(400).json({
                        status: false,
                        message: i18n_1.I18n.t('too_close_to_departure', req.lang) || 'Modification impossible: le départ est dans moins de 2 heures',
                        code: 400
                    });
                    return;
                }
            ...
            const seatCheck = await pgdb_1.default.oneOrNone(`SELECT id, status
                 FROM generated_trip_seat
                 WHERE id = $1 AND generated_trip_id = $2`, [new_seat_id, booking.generated_trip_id]);
            if (!seatCheck) {
                res.status(404).json({
                    status: false,
                    message: i18n_1.I18n.t('seat_not_found', req.lang) || 'Siège non trouvé',
                    code: 404
                });
                return;
            }
            if (seatCheck.status !== 'available') {
                res.status(400).json({
                    status: false,
                    message: i18n_1.I18n.t('seat_not_available', req.lang) || 'Ce siège n\'est pas disponible',
                    code: 400
                });
                return;
            }
            // Update booking with new seat
            await pgdb_1.default.none(`UPDATE booking
                 SET generated_trip_seat_id = $1,
                     updated_at = NOW()
                 WHERE id = $2`, [new_seat_id, booking_id]);
            // Mark old seat as available
            await pgdb_1.default.none(`UPDATE generated_trip_seat
                 SET status = 'available'
                 WHERE id = $1`, [booking.generated_trip_seat_id]);
            // Mark new seat as reserved
            await pgdb_1.default.none(`UPDATE generated_trip_seat
                 SET status = 'reserved'
                 WHERE id = $1`, [new_seat_id]);
            // Get updated booking
            const updatedBooking = await booking_repository_1.BookingRepository.findById(parseInt(booking_id));
            res.status(200).json({
                status: true,
                message: i18n_1.I18n.t('booking_modified', req.lang) || 'Réservation modifiée avec succès',
