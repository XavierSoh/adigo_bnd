...
const chat_tables_1 = require("../config/chat_tables");
class ChatRepository {
    ...
    static async createConversation(data) {
        ...
        const conversation = await pgdb_1.default.one(`INSERT INTO ${chat_tables_1.kConversations} (
                customer_id, subject, tags, status
            ) VALUES ($1, $2, $3, $4)
            RETURNING *`, [customer_id, subject || null, tags || null, 'open']);
    ...
    static async getConversationById(id) {
        try {
            const conversation = await pgdb_1.default.one(`SELECT c.*,
                    CONCAT(cust.first_name, ' ', cust.last_name) as customer_name,
                    u.login as admin_name,
                    COUNT(m.id) as message_count
                FROM ${chat_tables_1.kConversations} c
                LEFT JOIN customer cust ON c.customer_id = cust.id
                LEFT JOIN "users" u ON c.assigned_to = u.id
                LEFT JOIN ${chat_tables_1.kMessages} m ON c.id = m.conversation_id AND m.is_deleted = FALSE
                WHERE c.id = $1 AND c.is_deleted = FALSE
                GROUP BY c.id, cust.first_name, cust.last_name, u.login`, [id]);
            return conversation;
        }
        catch {
            return null;
     ...
     */
    static async getConversationsByCustomer(customerId, status) {
        const statusFilter = status ? 'AND c.status = $2' : '';
        const params = status ? [customerId, status] : [customerId];
        return await pgdb_1.default.manyOrNone(`SELECT c.*,
                CONCAT(cust.first_name, ' ', cust.last_name) as customer_name,
                u.login as admin_name,
                COUNT(m.id) as message_count
            FROM ${chat_tables_1.kConversations} c
     ...
     */
    static async getConversationsByAdmin(adminId, status) {
        const statusFilter = status ? 'AND c.status = $2' : '';
    ...
    static async updateConversation(id, data) {
        ...
        values.push(id);
        try {
            return await pgdb_1.default.one(`UPDATE ${chat_tables_1.kConversations}
                SET ${fields.join(', ')}
    ...
    static async assignConversation(conversationId, adminId) {
        const result = await pgdb_1.default.result(`UPDATE ${chat_tables_1.kConversations}
            SET assigned_to = $1, updated_at = CURRENT_TIMESTAMP
            WHERE id = $2 AND is_deleted = FALSE`, [adminId, conversationId]);
        return result.rowCount > 0;
    }
    /**
     * Supprimer une conversation (soft delete)
     */
    static async deleteConversation(id, deletedBy) {
        const result = await pgdb_1.default.result(`UPDATE ${chat_tables_1.kConversations}
            SET is_deleted = TRUE, deleted_at = CURRENT_TIMESTAMP, deleted_by = $1
            WHERE id = $2`, [deletedBy, id]);
        return result.rowCount > 0;
    }
    // ============================================
    // MESSAGES
    ...
    static async createMessage(data) {
        const { conversation_id, sender_type, sender_id, sender_name, message_type, content, metadata, } = data;
        const message = await pgdb_1.default.one(`INSERT INTO ${chat_tables_1.kMessages} (
        ...
        await pgdb_1.default.none(`UPDATE ${chat_tables_1.kConversations}
            SET last_message_at = CURRENT_TIMESTAMP,
                last_message_preview = $1,
                updated_at = CURRENT_TIMESTAMP,
                unread_count_${sender_type === 'customer' ? 'admin' : 'customer'} =
                    unread_count_${sender_type === 'customer' ? 'admin' : 'customer'} + 1
            WHERE id = $2`, [content.substring(0, 100), conversation_id]);
        return message;
    }
    /**
     * Récupérer tous les messages d'une conversation
     */
    static async getMessagesByConversation(conversationId, limit, offset) {
        const limitClause = limit ? `LIMIT ${limit}` : '';
        const offsetClause = offset ? `OFFSET ${offset}` : '';
        return await pgdb_1.default.manyOrNone(`SELECT * FROM ${chat_tables_1.kMessages}
            WHERE conversation_id = $1 AND is_deleted = FALSE
            ORDER BY created_at ASC
            ${limitClause} ${offsetClause}`, [conversationId]);
    }
    /**
    ...
    static async markMessageAsRead(messageId) {
        const result = await pgdb_1.default.result(`UPDATE ${chat_tables_1.kMessages}
            SET is_read = TRUE, read_at = CURRENT_TIMESTAMP
            WHERE id = $1 AND is_read = FALSE`, [messageId]);
        return result.rowCount > 0;
    }
    /**
     * Marquer tous les messages d'une conversation comme lus
     */
    static async markConversationAsRead(conversationId, forUser) {
        // Marquer les messages comme lus
        await pgdb_1.default.none(`UPDATE ${chat_tables_1.kMessages}
            SET is_read = TRUE, read_at = CURRENT_TIMESTAMP
            WHERE conversation_id = $1
              AND sender_type != $2
              AND is_read = FALSE`, [conversationId, forUser]);
     ...
     */
    static async updateMessageAI(messageId, aiData) {
        const fields = [];
        const values = [];
        let paramIndex = 1;
    ...
    static async createQuickReply(data) {
        const { title, action_type, payload, icon, color, display_order, requires_auth, user_role, created_by, } = data;
        return await pgdb_1.default.one(`INSERT INTO ${chat_tables_1.kQuickReplies} (
     ...
     */
    static async getActiveQuickReplies(userRole) {
        const roleFilter = userRole
            ? `AND (user_role IS NULL OR user_role = $1)`
            : '';
        const params = userRole ? [userRole] : [];
        return await pgdb_1.default.manyOrNone(`SELECT * FROM ${chat_tables_1.kQuickReplies}
    ...
    static async getQuickReplyById(id) {
        try {
            return await pgdb_1.default.one(`SELECT * FROM ${chat_tables_1.kQuickReplies} WHERE id = $1`, [id]);
        }
        catch {
            return null;
        }
     ...
     */
    static async getQuickRepliesByIds(ids) {
        if (ids.length === 0)
            return [];
        return await pgdb_1.default.manyOrNone(`SELECT * FROM ${chat_tables_1.kQuickReplies}
            WHERE id = ANY($1) AND is_active = TRUE
            ORDER BY display_order ASC`, [ids]);
    }
    // ============================================
    // AI RESPONSES
    // ============================================
     ...
     */
    static async createAIResponse(data) {
        const { intent, keywords, pattern, response_template, response_type, language, quick_reply_ids, priority, confidence_threshold, trigger_on_first_message, max_uses_per_conversation, created_by, } = data;
        return await pgdb_1.default.one(`INSERT INTO ${chat_tables_1.kAIResponses} (
     ...
     */
    static async getActiveAIResponses() {
        return await pgdb_1.default.manyOrNone(`SELECT * FROM ${chat_tables_1.kAIResponses}
     ...
     */
    static async getAIResponsesByIntent(intent) {
        return await pgdb_1.default.manyOrNone(`SELECT * FROM ${chat_tables_1.kAIResponses}
     ...
     */
    static async incrementAIResponseUsage(id) {
        const result = await pgdb_1.default.result(`UPDATE ${chat_tables_1.kAIResponses}
    ...
    static async getChatStatistics() {
        const stats = await pgdb_1.default.one(`SELECT
            ...
            FROM ${chat_tables_1.kConversations}
            WHERE is_deleted = FALSE`);
        const messageStats = await pgdb_1.default.one(`SELECT
