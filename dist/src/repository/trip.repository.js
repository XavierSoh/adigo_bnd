...
class TripRepository {
    static async create(trip) {
        const t = await pgdb_1.default.tx(async (tx) => {
            try {
                let recurrencePatternId = null;
                // Create recurrence pattern if provided
                if (trip.recurrence_pattern) {
                    const recurrenceResult = await tx.one(`INSERT INTO ${table_names_1.kRecurrencePattern} (
                ...
                const result = await tx.one(`INSERT INTO ${table_names_1.kTrip} (
    ...
    static async findById(id) {
        try {
            const trip = await pgdb_1.default.oneOrNone(`SELECT t.*, 
                 ...
                 FROM ${table_names_1.kTrip} t
                 LEFT JOIN ${table_names_1.kRecurrencePattern} rp ON t.recurrence_pattern_id = rp.id
        ...
        const t = await pgdb_1.default.tx(async (tx) => {
            try {
                ...
                if (trip.recurrence_pattern) {
                    ...
                    const existingTrip = await tx.oneOrNone(`SELECT recurrence_pattern_id FROM ${table_names_1.kTrip} WHERE id = $1`, [id]);
                    if (existingTrip?.recurrence_pattern_id) {
                        // Update existing pattern
                        await tx.none(`UPDATE ${table_names_1.kRecurrencePattern} SET 
                ...
                // Update trip
                const result = await tx.oneOrNone(`UPDATE ${table_names_1.kTrip} SET 
    ...
    static async softDelete(id, deleted_by) {
        try {
            const result = await pgdb_1.default.result(`UPDATE ${table_names_1.kTrip} SET is_deleted = TRUE, deleted_at = NOW(), deleted_by = $2 
    ...
    static async restore(id) {
        try {
            const result = await pgdb_1.default.result(`UPDATE ${table_names_1.kTrip} SET is_deleted = FALSE, deleted_at = NULL, deleted_by = NULL 
    ...
    static async delete(id) {
        const t = await pgdb_1.default.tx(async (tx) => {
            try {
                ...
                const trip = await tx.oneOrNone(`SELECT recurrence_pattern_id FROM ${table_names_1.kTrip} WHERE id = $1`, [id]);
                // Delete trip
                const result = await tx.result(`DELETE FROM ${table_names_1.kTrip} WHERE id = $1`, [id]);
                if (result.rowCount === 0) {
                    throw new Error('Trip not found');
                }
                // Delete associated recurrence pattern if exists
                if (trip?.recurrence_pattern_id) {
                    await tx.none(`DELETE FROM ${table_names_1.kRecurrencePattern} WHERE id = $1`, [trip.recurrence_pattern_id]);
                }
    ...
    static async findAllByAgency(agencyId, isDeleted = false) {
        try {
            const trips = await pgdb_1.default.any(`SELECT t.id,
                        t.departure_city,
                        t.arrival_city,
                        t.departure_time,
                        t.arrival_time,
                 ...
                 FROM ${table_names_1.kTrip} t
                 LEFT JOIN ${table_names_1.kRecurrencePattern} rp ON t.recurrence_pattern_id = rp.id
                 LEFT JOIN ${table_names_1.kBus} b ON b.id = t.bus_id
                 WHERE t.agency_id = $1 AND t.is_deleted = $2
                 ORDER BY t.departure_time ASC`, [agencyId, isDeleted]);
    ...
    static async findByRoute(departureCity, arrivalCity, departureDate) {
        try {
            let query = `
                SELECT t.*, 
                        rp.type as recurrence_type, 
            ...
            const trips = await pgdb_1.default.any(query, params);
