...
class DashboardRepository {
     ...
     */
    static async getDashboardData(agencyId) {
        try {
    ...
    static async getStats(agencyId) {
        ...
        const currentStats = await pgdb_1.default.one(`
            SELECT
        ...
        const previousStats = await pgdb_1.default.one(`
            SELECT
                COALESCE(SUM(b.total_price), 0) as total_revenue,
                COUNT(DISTINCT b.id) as total_bookings,
                COUNT(DISTINCT gt.id) as total_trips,
        ...
        return {
            ...
            revenueChange: calculateChange(parseFloat(currentStats.total_revenue) || 0, parseFloat(previousStats.total_revenue) || 0),
            bookingsChange: calculateChange(parseInt(currentStats.total_bookings) || 0, parseInt(previousStats.total_bookings) || 0),
            tripsChange: calculateChange(parseInt(currentStats.total_trips) || 0, parseInt(previousStats.total_trips) || 0),
            customersChange: calculateChange(parseInt(currentStats.total_customers) || 0, parseInt(previousStats.total_customers) || 0)
        };
    }
    /**
     * Get revenue chart data for the last N days
     */
    static async getRevenueChartData(agencyId, days) {
        const agencyFilter = agencyId ? 'AND ag.id = $2' : '';
        const params = agencyId ? [days, agencyId] : [days];
        const data = await pgdb_1.default.manyOrNone(`
            SELECT
     ...
     * Get occupation data
     */
    static async getOccupationData(agencyId) {
        const agencyFilter = agencyId ? 'AND ag.id = $1' : '';
        const params = agencyId ? [agencyId] : [];
        const data = await pgdb_1.default.one(`
            SELECT
     ...
     * Get upcoming trips
     */
    static async getUpcomingTrips(agencyId, limit) {
        const agencyFilter = agencyId ? `AND ag.id = $2` : '';
        const params = agencyId ? [limit, agencyId] : [limit];
        const trips = await pgdb_1.default.manyOrNone(`
            SELECT
                gt.id,
                gt.trip_id,
                t.departure_city,
     ...
     * Get recent bookings
     */
    static async getRecentBookings(agencyId, limit) {
        const agencyFilter = agencyId ? `AND ag.id = $2` : '';
        const params = agencyId ? [limit, agencyId] : [limit];
        const bookings = await pgdb_1.default.manyOrNone(`
            SELECT
                b.id,
                b.booking_reference,
