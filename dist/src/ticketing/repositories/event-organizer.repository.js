...
exports.EventOrganizerRepository = void 0;
...
class EventOrganizerRepository {
    ...
    static async create(organizer, createdBy) {
        try {
            const result = await pgdb_1.default.one(`INSERT INTO ${table_names_1.kEventOrganizer} (
            ...
            await pgdb_1.default.none(`UPDATE ${table_names_1.kCustomer} SET is_organizer = TRUE WHERE id = $1`, [organizer.customer_id]);
    ...
    static async findByCustomerId(customerId) {
        try {
            const organizer = await pgdb_1.default.oneOrNone(`SELECT ${this.BASE_SELECT}
                ${this.BASE_JOINS}
                WHERE o.customer_id = $1 AND o.is_deleted = FALSE`, [customerId]);
            if (!organizer) {
    ...
    static async findById(id) {
        try {
            const organizer = await pgdb_1.default.oneOrNone(`SELECT ${this.BASE_SELECT}
                ${this.BASE_JOINS}
                WHERE o.id = $1 AND o.is_deleted = FALSE`, [id]);
            if (!organizer) {
    ...
    static async findAll(includeDeleted = false) {
        try {
            ...
            const organizers = await pgdb_1.default.any(`SELECT ${this.BASE_SELECT}
                ${this.BASE_JOINS}
                ${whereClause}
    ...
    // Update organizer
    static async update(id, organizer) {
        try {
    ...
    static async updateVerificationStatus(id, status, verifiedBy, verificationNotes) {
        try {
            ...
            const result = await pgdb_1.default.oneOrNone(`UPDATE ${table_names_1.kEventOrganizer} SET
                ...
                RETURNING *`, [status, verifiedBy, verifiedAt, verificationNotes, id]);
            if (!result) {
                return { status: false, message: "Organisateur non trouvé", code: 404 };
            }
    ...
    static async search(filters) {
        try {
            ...
            const organizers = await pgdb_1.default.any(query, params);
    ...
    static async getStatistics() {
        try {
            const stats = await pgdb_1.default.one(`SELECT
                ...
                FROM ${table_names_1.kEventOrganizer}
    ...
    static async softDelete(id, deletedBy) {
        try {
            const result = await pgdb_1.default.result(`UPDATE ${table_names_1.kEventOrganizer} SET
                    is_deleted = TRUE,
                    deleted_at = NOW(),
                    deleted_by = $2,
                    updated_at = NOW()
...
exports.EventOrganizerRepository = EventOrganizerRepository;
EventOrganizerRepository.BASE_SELECT = `
        o.*,
        json_build_object(
            'id', c.id,
    ...
    `;
EventOrganizerRepository.BASE_JOINS = `
        FROM ${table_names_1.kEventOrganizer} o
        LEFT JOIN ${table_names_1.kCustomer} c ON o.customer_id = c.id
    `;
