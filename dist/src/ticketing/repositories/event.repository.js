...
class EventRepository {
    // Create new event
    static async create(event, createdBy) {
        try {
            const result = await pgdb_1.default.one(`INSERT INTO ${table_names_1.kEvent} (
    ...
    static async findById(id) {
        try {
            const event = await pgdb_1.default.oneOrNone(`SELECT ${this.BASE_SELECT}
                ${this.BASE_JOINS}
                WHERE e.id = $1 AND e.is_deleted = FALSE`, [id]);
            if (!event) {
    ...
    static async findByEventCode(eventCode) {
        try {
            const event = await pgdb_1.default.oneOrNone(`SELECT ${this.BASE_SELECT}
                ${this.BASE_JOINS}
                WHERE e.event_code = $1 AND e.is_deleted = FALSE`, [eventCode]);
            if (!event) {
                return { status: false, message: "Événement non trouvé", code: 404 };
            }
            return { status: true, message: "Événement trouvé", body: event, code: 200 };
    ...
    static async findAll(includeDeleted = false) {
        try {
            ...
            const events = await pgdb_1.default.any(`SELECT ${this.BASE_SELECT}
                ${this.BASE_JOINS}
                ${whereClause}
    ...
    static async findByOrganizer(organizerId) {
        try {
            const events = await pgdb_1.default.any(`SELECT ${this.BASE_SELECT}
                ...
                ORDER BY e.event_date DESC`, [organizerId]);
    ...
    static async findByCategory(categoryId) {
        try {
            const events = await pgdb_1.default.any(`SELECT ${this.BASE_SELECT}
                ...
                ORDER BY e.event_date ASC`, [categoryId]);
    ...
    // Publish event
    static async publish(id, publishData) {
        try {
            const result = await pgdb_1.default.oneOrNone(`UPDATE ${table_names_1.kEvent} SET
    ...
    // Approve event (admin)
    static async approve(id, validatedBy, validationNotes) {
        try {
            const result = await pgdb_1.default.oneOrNone(`UPDATE ${table_names_1.kEvent} SET
                    status = 'published',
                ...
                RETURNING *`, [validatedBy, validationNotes, id]);
            if (!result) {
    ...
    // Reject event (admin)
    static async reject(id, validatedBy, validationNotes) {
        try {
            const result = await pgdb_1.default.oneOrNone(`UPDATE ${table_names_1.kEvent} SET
                    status = 'draft',
    ...
    static async cancel(id, cancellationReason) {
        try {
            const result = await pgdb_1.default.oneOrNone(`UPDATE ${table_names_1.kEvent} SET
                ...
                RETURNING *`, [cancellationReason, id]);
            if (!result) {
                return { status: false, message: "Événement non trouvé", code: 404 };
            }
    ...
    static async search(params) {
        try {
            ...
            const events = await pgdb_1.default.any(query, queryParams);
    ...
    // Get statistics
    static async getStatistics(organizerId) {
        try {
            let query = `
                SELECT
                    COUNT(*) as total,
    ...
    static async softDelete(id, deletedBy) {
        try {
            const result = await pgdb_1.default.result(`UPDATE ${table_names_1.kEvent} SET
                    is_deleted = TRUE,
                    deleted_at = NOW(),
                    deleted_by = $2,
                    updated_at = NOW()
...
EventRepository.BASE_SELECT = `
    ...
    `;
EventRepository.BASE_JOINS = `
        FROM ${table_names_1.kEvent} e
        LEFT JOIN ${table_names_1.kEventCategory} cat ON e.category_id = cat.id
        LEFT JOIN ${table_names_1.kEventOrganizer} org ON e.organizer_id = org.id
    `;
