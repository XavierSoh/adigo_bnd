...
class EventCategoryRepository {
    ...
    static async findById(id) {
        try {
            const category = await pgdb_1.default.oneOrNone(`SELECT * FROM ${table_names_1.kEventCategory} WHERE id = $1 AND is_deleted = FALSE`, [id]);
            if (!category) {
    ...
    // Find all categories
    static async findAll(includeDeleted = false, activeOnly = true) {
        try {
            let query = `SELECT * FROM ${table_names_1.kEventCategory}`;
            const conditions = [];
            if (!includeDeleted) {
            ...
            const categories = await pgdb_1.default.any(query);
    ...
    static async softDelete(id, deletedBy) {
        try {
            const result = await pgdb_1.default.result(`UPDATE ${table_names_1.kEventCategory} SET
                    is_deleted = TRUE,
                    deleted_at = NOW(),
                    deleted_by = $2,
                    updated_at = NOW()
    ...
    static async getStatistics() {
        try {
            const stats = await pgdb_1.default.any(`SELECT
                ...
                FROM ${table_names_1.kEventCategory} c
    ...
    static async reorder(categoryOrders) {
        try {
            ...
            for (const item of categoryOrders) {
                await pgdb_1.default.none(`UPDATE ${table_names_1.kEventCategory} SET display_order = $1, updated_at = NOW() WHERE id = $2`, [item.display_order, item.id]);
            }
