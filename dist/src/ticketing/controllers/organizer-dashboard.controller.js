...
exports.OrganizerDashboardController = void 0;
...
class OrganizerDashboardController {
    ...
    static async getStats(req, res) {
        try {
            ...
            const revenue = await pgdb_1.default.one(`
                SELECT
            ...
            const premiumServices = await pgdb_1.default.one(`
                SELECT
    ...
    static async getSales(req, res) {
        try {
            const lang = req.lang || 'en';
            const organizerId = parseInt(req.params.id);
            const sales = await pgdb_1.default.any(`
                SELECT
                    t.*,
                    json_build_object(
                        'id', e.id,
            ...
            res.status(200).json({
                status: true,
                message: i18n_1.I18n.t('sales_retrieved', lang),
     ...
     */
    static async getValidatedTickets(req, res) {
        try {
            const lang = req.lang || 'en';
            const organizerId = parseInt(req.params.id);
    ...
    static async getRevenue(req, res) {
        try {
            ...
            const organizerId = parseInt(req.params.id);
            // Revenue by event
            const revenueByEvent = await pgdb_1.default.any(`
                SELECT
                    e.id,
                    e.title,
                    e.event_code,
            ...
            res.status(200).json({
                status: true,
                message: i18n_1.I18n.t('revenue_retrieved', lang),
...
exports.OrganizerDashboardController = OrganizerDashboardController;
