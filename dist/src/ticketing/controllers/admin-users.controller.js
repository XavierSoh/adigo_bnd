...
class AdminUsersController {
    ...
    static async getAllUsers(req, res) {
        try {
            ...
            query += `
                ...
                LIMIT $1 OFFSET $2
            `;
            const users = await pgdb_1.default.any(query, [limitNum, offsetNum]);
            // Get total count
            let countQuery = `SELECT COUNT(*)::int AS total FROM customer`;
            if (include_deleted !== 'true') {
                countQuery += ` WHERE is_deleted = FALSE`;
            }
            const { total } = await pgdb_1.default.one(countQuery);
            res.status(200).json({
                status: true,
                message: i18n_1.I18n.t('users_retrieved', lang),
                body: {
                    users,
                    pagination: {
                        total,
    ...
    static async searchUsers(req, res) {
        try {
            ...
            const users = await pgdb_1.default.any(query, params);
            res.status(200).json({
                status: true,
                message: i18n_1.I18n.t('search_results', lang),
    ...
    static async getUserById(req, res) {
        try {
            ...
            if (!user) {
                res.status(404).json({
                    status: false,
                    message: i18n_1.I18n.t('user_not_found', lang),
                    code: 404
                });
                return;
            }
            res.status(200).json({
                status: true,
                message: i18n_1.I18n.t('user_found', lang),
                body: user,
                code: 200
            });
        }
    ...
    static async updateUserStatus(req, res) {
        try {
            ...
            const updatedUser = await pgdb_1.default.one(`
                UPDATE customer
            ...
            res.status(200).json({
                status: true,
                message: is_active
                    ? i18n_1.I18n.t('user_activated', lang)
                    : i18n_1.I18n.t('user_deactivated', lang),
                body: updatedUser,
                code: 200
            });
        }
    ...
    static async getUserTickets(req, res) {
        try {
            ...
            res.status(200).json({
                status: true,
                message: i18n_1.I18n.t('tickets_retrieved', lang),
    ...
    static async getUserWallet(req, res) {
        try {
            const lang = req.lang || 'en';
            const userId = parseInt(req.params.id);
            const wallet = await pgdb_1.default.oneOrNone(`
                SELECT
                    w.*,
            ...
            if (!wallet) {
                res.status(404).json({
                    status: false,
                    message: i18n_1.I18n.t('wallet_not_found', lang),
                    code: 404
                });
                return;
            }
            res.status(200).json({
                status: true,
                message: i18n_1.I18n.t('wallet_retrieved', lang),
                body: wallet,
                code: 200
            });
        }
