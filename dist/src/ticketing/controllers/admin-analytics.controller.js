...
class AdminAnalyticsController {
    ...
    static async getDashboard(req, res) {
        try {
            ...
            // Users stats
            const usersStats = await pgdb_1.default.one(`
                SELECT
            ...
            `);
            // Events stats
            const eventsStats = await pgdb_1.default.one(`
                SELECT
            ...
            `);
            // Tickets stats
            const ticketsStats = await pgdb_1.default.one(`
                SELECT
            ...
            `);
            // Organizers stats
            const organizersStats = await pgdb_1.default.one(`
                SELECT
            ...
            const recentActivity = await pgdb_1.default.one(`
                SELECT
            ...
            res.status(200).json({
                status: true,
                message: i18n_1.I18n.t('dashboard_retrieved', lang),
                body: {
                    users: usersStats,
                    events: eventsStats,
    ...
    static async getSalesTrends(req, res) {
        try {
            ...
            const trends = await pgdb_1.default.any(`
                SELECT
            ...
            res.status(200).json({
                status: true,
                message: i18n_1.I18n.t('trends_retrieved', lang),
                body: { trends },
                code: 200
            });
        }
     ...
     */
    static async getTopOrganizers(req, res) {
        try {
            const lang = req.lang || 'en';
            const { limit = 10 } = req.query;
            const topOrganizers = await pgdb_1.default.any(`
                SELECT
                    eo.id,
                    eo.name,
                    eo.email,
            ...
            res.status(200).json({
                status: true,
                message: i18n_1.I18n.t('top_organizers_retrieved', lang),
    ...
    static async getTopEvents(req, res) {
        try {
            ...
            const topEvents = await pgdb_1.default.any(`
                SELECT
                    e.id,
                    e.title,
            ...
            res.status(200).json({
                status: true,
                message: i18n_1.I18n.t('top_events_retrieved', lang),
    ...
    static async getRevenueBreakdown(req, res) {
        try {
            ...
            `);
            // Revenue by source
            const bySource = await pgdb_1.default.one(`
                SELECT
            ...
            res.status(200).json({
                status: true,
                message: i18n_1.I18n.t('revenue_breakdown_retrieved', lang),
                body: {
