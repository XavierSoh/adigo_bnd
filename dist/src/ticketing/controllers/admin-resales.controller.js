...
exports.AdminResalesController = void 0;
const i18n_1 = require("../../utils/i18n");
const pgdb_1 = __importDefault(require("../../config/pgdb"));
class AdminResalesController {
    /**
     * Get all resales
     * GET /v1/api/ticketing/admin/resales
     */
    static async getAllResales(req, res) {
        try {
            ...
            const resales = await pgdb_1.default.any(query, params);
            // Get total count
            ...
            res.status(200).json({
                status: true,
                message: i18n_1.I18n.t('resales_retrieved', lang),
                body: {
                    resales,
                    pagination: {
                        total,
     ...
     */
    static async getResaleStats(req, res) {
        try {
            const lang = req.lang || 'en';
     ...
     */
    static async approveResale(req, res) {
        try {
            const lang = req.lang || 'en';
            const resaleId = parseInt(req.params.id);
            const adminId = req.userId;
            const updatedResale = await pgdb_1.default.one(`
            ...
            res.status(200).json({
                status: true,
                message: i18n_1.I18n.t('resale_approved', lang),
                body: updatedResale,
                code: 200
            });
        }
     ...
     */
    static async rejectResale(req, res) {
        try {
            const lang = req.lang || 'en';
            const resaleId = parseInt(req.params.id);
            ...
            res.status(200).json({
                status: true,
                message: i18n_1.I18n.t('resale_rejected', lang),
                body: updatedResale,
                code: 200
            });
        }
     ...
     */
    static async deleteResale(req, res) {
        try {
            const lang = req.lang || 'en';
            const resaleId = parseInt(req.params.id);
            const { reason } = req.body;
            const adminId = req.userId;
            const deletedResale = await pgdb_1.default.one(`
                UPDATE event_ticket_resale
            ...
            res.status(200).json({
                status: true,
                message: i18n_1.I18n.t('resale_deleted', lang),
                body: deletedResale,
                code: 200
            });
        }
...
exports.AdminResalesController = AdminResalesController;
