...
export interface PremiumServicesRequest {
    ...
    // Design (MANDATORY)
    designRequired: boolean;  // Always true
...
export class PremiumPaymentService {
    ...
    static async calculateTotalCost(
        request: PremiumServicesRequest
    ): Promise<PremiumServicesCostBreakdown> {
        try {
            const breakdown: PremiumServicesCostBreakdown = {
                design: 0,
                boostHomepage: 0,
                boostCategory: 0,
                fieldService: 0,
            ...
            const designPricing = await PremiumDesignService.calculateDesignPrice(
                event.total_tickets
            );
            ...
            if (request.boostDurationDays) {
                ...
                if (request.boostHomepage) {
                    const homepagePricing = await VisibilityBoostService.getBoostPricing(
                        'homepage',
                        request.boostDurationDays
                    );
                    if (homepagePricing) {
                ...
                if (request.boostCategory) {
                    const categoryPricing = await VisibilityBoostService.getBoostPricing(
                        'category',
                        request.boostDurationDays
                    );
                    if (categoryPricing) {
     ...
     */
    static async applyPremiumServices(
        request: PremiumServicesRequest,
        breakdown: PremiumServicesCostBreakdown
    ): Promise<boolean> {
        try {
            ...
            if (request.boostHomepage || request.boostCategory) {
                await VisibilityBoostService.applyBoost(request.eventId, {
