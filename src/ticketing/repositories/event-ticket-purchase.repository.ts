...
import {
    ...
    EventTicketValidateDto,
    EventTicketRefundDto
} from "../models/event-ticket-purchase.model";
import ResponseModel from "../../models/response.model";

export class EventTicketPurchaseRepository {

    private static readonly TABLE = 'event_ticket';
    ...
    static async confirmPayment(
        ...
        try {
            const result = await pgpDb.one(
                ...
                WHERE id = $1
                RETURNING *`,
                [ticketId, qrCodeData, qrCodeImage, walletTransactionId]
            );
    ...
    static async validateTicket(validation: EventTicketValidateDto): Promise<ResponseModel> {
        try {
            ...
            const result = await pgpDb.one(
                ...
                WHERE id = $1
                RETURNING *`,
                [ticket.id, validation.validated_by, validation.validation_method || 'scanner_device']
            );
    ...
    static async findByCustomer(customerId: number, filters?: any): Promise<ResponseModel> {
        try {
            let query = `
                SELECT
                    t.*,
    ...
    static async refund(ticketId: number, refund: EventTicketRefundDto): Promise<ResponseModel> {
        try {
            const result = await pgpDb.one(
                ...
                WHERE id = $1
                RETURNING *`,
                [ticketId, refund.refund_reason, refund.refund_processed_by]
            );
