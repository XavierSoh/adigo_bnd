...
export interface BookingProposalMetadata {
    ...
    expires_at?: string; // Expiration de l'offre
}
...
export interface Conversation {
    ...
    // Participants
    customer_id: number;
    assigned_to?: number | null; // Admin assigné
    ...
    // Métadonnées
    subject?: string;
    tags?: string[]; // ['support', 'booking', 'payment', etc.]
...
export interface Message {
    ...
    // Métadonnées selon le type de message
...
export interface QuickReply {
    ...
    // Payload à envoyer lors du clic
...
export interface AIResponse {
    ...
    // Réponse
    response_template: string; // Template avec variables: "Bonjour {{name}}!"
    response_type: ResponseType;
    language: 'fr' | 'en'; // Langue de la réponse (français par défaut, anglais sinon)

    // Quick replies associées
...
export interface CreateAIResponseDTO {
    ...
    response_type?: ResponseType;
    language?: 'fr' | 'en'; // Par défaut: 'en' (anglais)
    quick_reply_ids?: number[];
    priority?: number;
