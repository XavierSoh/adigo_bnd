...
export const kConversations = "conversations";
export const kMessages = "messages";
export const kQuickReplies = "quick_replies";
export const kAIResponses = "ai_responses";
...
export const chatTables = [
    ...
    {
        query: `CREATE TABLE IF NOT EXISTS ${kConversations} (
    ...
    // Index pour conversations
    {
        query: `CREATE INDEX IF NOT EXISTS idx_conversations_customer
                ON ${kConversations}(customer_id) WHERE is_deleted = FALSE;`
    },
    {
        query: `CREATE INDEX IF NOT EXISTS idx_conversations_assigned
                ON ${kConversations}(assigned_to) WHERE is_deleted = FALSE;`
    },
    {
        query: `CREATE INDEX IF NOT EXISTS idx_conversations_status
                ON ${kConversations}(status) WHERE is_deleted = FALSE;`
    },

    // Table des messages
    {
        query: `CREATE TABLE IF NOT EXISTS ${kMessages} (
            id SERIAL PRIMARY KEY,
            conversation_id INT NOT NULL REFERENCES ${kConversations}(id) ON DELETE CASCADE,
    ...
    // Index pour messages
    {
        query: `CREATE INDEX IF NOT EXISTS idx_messages_conversation
                ON ${kMessages}(conversation_id) WHERE is_deleted = FALSE;`
    },
    {
        query: `CREATE INDEX IF NOT EXISTS idx_messages_created
                ON ${kMessages}(created_at DESC);`
    },
    {
        query: `CREATE INDEX IF NOT EXISTS idx_messages_ai_intent
                ON ${kMessages}(ai_intent) WHERE ai_processed = TRUE;`
    },

    // Table des boutons d'action rapide (Quick Replies)
    {
        query: `CREATE TABLE IF NOT EXISTS ${kQuickReplies} (
            id SERIAL PRIMARY KEY,
    ...
    {
        query: `CREATE TABLE IF NOT EXISTS ${kAIResponses} (
            id SERIAL PRIMARY KEY,
    ...
    // Index pour AI responses
    {
        query: `CREATE INDEX IF NOT EXISTS idx_ai_responses_intent
                ON ${kAIResponses}(intent) WHERE is_active = TRUE;`
    },
    {
        query: `CREATE INDEX IF NOT EXISTS idx_ai_responses_priority
                ON ${kAIResponses}(priority DESC) WHERE is_active = TRUE;`
    }
];
