...
export class ChatAIService {
     ...
     */
    private static intentKeywords: Record<AIIntent, string[]> = {
        greeting: [
            // Français
            'bonjour',
            'salut',
     ...
     */
    static async analyzeMessage(message: Message): Promise<AIProcessingResult> {
        ...
        const aiResponses = await ChatRepository.getAIResponsesByIntent(intent);
        ...
        if (matchingResponse?.quick_reply_ids && matchingResponse.quick_reply_ids.length > 0) {
            quickReplies = await ChatRepository.getQuickRepliesByIds(
                matchingResponse.quick_reply_ids
            );
        } else {
        ...
        let bookingProposal: BookingProposalMetadata | undefined;
        if (intent === 'booking') {
        ...
        if (matchingResponse) {
            await ChatRepository.incrementAIResponseUsage(matchingResponse.id);
        }
     ...
     */
    private static detectIntent(content: string): AIIntent {
        ...
        for (const [intent, keywords] of Object.entries(this.intentKeywords)) {
            ...
            if (matchCount > 0) {
                scores[intent as AIIntent] = matchCount;
            }
        }
     ...
     */
    private static calculateConfidence(content: string, intent: AIIntent): number {
     ...
     */
    private static interpolateTemplate(template: string, message: Message): string {
        let result = template;
     ...
     */
    private static async getQuickRepliesForIntent(
        intent: AIIntent
    ): Promise<QuickReply[]> {
        // Mapping intent -> quick reply actions
        const intentToActions: Record<AIIntent, string[]> = {
        ...
        const allQuickReplies = await ChatRepository.getActiveQuickReplies('customer');
     ...
     */
    private static async generateBookingProposal(
        content: string
    ): Promise<BookingProposalMetadata | undefined> {
     ...
     */
    private static extractDestinations(content: string): {
        from?: string;
        to?: string;
    } {
     ...
     */
    static async processIncomingMessage(message: Message): Promise<void> {
        // Analyser le message
        const aiResult = await this.analyzeMessage(message);

        // Mettre à jour le message avec les données IA
        await ChatRepository.updateMessageAI(message.id, {
            ai_processed: true,
     ...
     */
    static async getAIResponse(messageId: number): Promise<AIProcessingResult | null> {
        const message = await ChatRepository.getMessagesByConversation(messageId);
