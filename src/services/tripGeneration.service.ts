...
export class TripGenerationService {
    ...
    // Public so TripRepository.findByRoute can reuse the exact same
    // recurrence-matching logic instead of re-deriving it - see that
    // method's doc comment.
    isValidDateForTrip(trip: any, date: Date): boolean {
    ...
    // Public (and returning the row instead of void) so TripRepository.
    // findByRoute can materialize the specific day's bookable instance
    // on demand for a date beyond the daily cron's own ~7-day generation
    // window, then read its real seat count - without this, a search far
    // enough in advance could confirm a route *runs* that day (via
    // isValidDateForTrip) but have no generated_trip to actually check
    // capacity against or book at all yet. Idempotent regardless of
    // caller: the unique (trip_id, actual_departure_time) constraint means
    // a date already generated by the cron is simply fetched, never
    // duplicated.
    async ensureInstanceExists(trip: any, date: Date) {
        return this.createTripInstance(trip, date);
    }

    private async createTripInstance(trip: any, date: Date) {
        ...
        try {
            return await prismaDb.generated_trip.create({
                data: {
                    trip_id: trip.id,
            ...
            // like the original; anything else still propagates. Fetch and
            // return the row that already won the race instead (needed by
            // ensureInstanceExists's callers - createTripInstance's own
            // original caller, the bulk cron job, never used the return
            // value at all).
            if (error instanceof Prisma.PrismaClientKnownRequestError && error.code === 'P2002') {
                return prismaDb.generated_trip.findFirst({
                    where: { trip_id: trip.id, actual_departure_time: departureTime },
                });
            }
            throw error;
        }
