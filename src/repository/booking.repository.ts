...
import { kBooking, kTrip, kUsers, kSeat, kBus, kCustomer, kGeneratedTrip, kGeneratedTripSeat, kAgency } from "../utils/table_names";
...
export class BookingRepository { 
    ...
    static async addPassenger(bookingId: number, name?: string, phone?: string, document_type?: string, document_number?: string): Promise<void> {
        await pgpDb.none(
            `INSERT INTO booking_passenger (booking_id, name, phone, document_type, document_number) VALUES ($1, $2, $3, $4, $5)`,
            [bookingId, name || null, phone || null, document_type || null, document_number || null]
        );
    }
    ...
    static async getStatisticsByPaymentMethod(agencyId?: number): Promise<ResponseModel> {
        try {
            ...
            if (agencyId) {
                query += `
                    INNER JOIN ${kTrip} t ON b.trip_id = t.id
            ...
            const result = await pgpDb.any(query, params);
    ...
    static async delete(id: number): Promise<ResponseModel> {
        try {
            const result = await pgpDb.result(
                `DELETE FROM ${kBooking} WHERE id = $1`,
                [id]
            );
    ...
    static async findByTripAndStatus(tripId: number, status: string): Promise<ResponseModel> {
        try {
            const bookings = await pgpDb.any(
    ...
    static async getBookedSeatIds(tripId: number): Promise<ResponseModel> {
        try {
            const result = await pgpDb.any(
                `SELECT generated_trip_seat_id FROM ${kBooking}
                WHERE trip_id = $1 AND status = 'confirmed' AND is_deleted = FALSE`,
                [tripId]
            );

            const seatIds = result.map((row: any) => row.generated_trip_seat_id);
    ...
    static async search(filters: {
        ...
        try {
            let query = `
                SELECT b.* FROM ${kBooking} b
                INNER JOIN ${kTrip} t ON b.trip_id = t.id
                INNER JOIN "${kUsers}" u ON b.customer_id = u.id
                WHERE b.is_deleted = FALSE
            `;
            ...
            const bookings = await pgpDb.any(query, params);
 ...
 //********************************************************************************************* */
     ...
     */
    private static normalizeNumericFields(booking: any): any {
    ...
    private static readonly BASE_JOINS = `
        FROM ${kBooking} b
        LEFT JOIN ${kCustomer} c ON b.customer_id = c.id
        LEFT JOIN ${kGeneratedTrip} gt ON b.generated_trip_id = gt.id
        LEFT JOIN ${kGeneratedTripSeat} gts ON b.generated_trip_seat_id = gts.id
        LEFT JOIN ${kBus} bs ON bs.id = gt.bus_id
        LEFT JOIN ${kSeat} s ON s.id = gts.seat_id
        LEFT JOIN ${kTrip} t ON t.id = gt.trip_id
        LEFT JOIN ${kAgency} ag ON ag.id = t.agency_id
    ...
    static async findByAgency(agencyId: number): Promise<ResponseModel> {
        try {
            const bookings = await pgpDb.any(
                ...
                ${this.BASE_JOINS}
                LEFT JOIN ${kUsers} u ON u.id = b.created_by
                WHERE t.agency_id = $1 AND b.is_deleted = FALSE
                ORDER BY b.booking_date DESC`,
                [agencyId]
            );
    ...
    static async findById(id: number): Promise<ResponseModel> {
        try {
            const booking = await pgpDb.oneOrNone(
                `SELECT ${this.BASE_SELECT}
                ${this.BASE_JOINS}
    ...
    static async softDeleteByGroup(groupId: string): Promise<void> {
        await pgpDb.none(`UPDATE booking SET is_deleted = TRUE WHERE group_id = $1`, [groupId]);
    }
}